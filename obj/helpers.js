...
(function() {
  ...
  C1964jsHelpers = function(core, isLittleEndian) {
    ...
    this.setVAddr = function(i) {
      return "var vAddr=(" + this.RS(i) + "+" + this.soffset_imm(i) + ")>>0;";
    };
    this.fn = function(i) {
      return i & 0x3f;
    };
    ...
    this.sLogic = function(i, n) {
      if (this.rd(i) === this.rs(i)) {
        return this.tRDH(i) + "=(" + this.tRD(i) + n + "=" + this.RT(i) + ")>>31;";
      } else {
        return this.tRDH(i) + "=(" + this.tRD(i) + "=" + this.RS(i) + n + this.RT(i) + ")>>31;";
      }
    };
    ...
    this.inter_mtc0 = function(r, f, rt, isDelaySlot, pc, cp0, interrupts) {
      switch (f) {
        case consts.CAUSE:
          cp0[f] &= ~0x300;
          cp0[f] |= r[rt] & 0x300;
          if (r[rt] & 0x300 ? (cp0[consts.CAUSE] & cp0[consts.STATUS] & 0x0000FF00) !== 0 : void 0) {
    ...
    this.inter_daddi = function(r, h, i) {
      ...
      rtres = rs1.add(imm);
      r[this.rt(i)] = rtres.getLowBits();
      h[this.rt(i)] = rtres.getHighBits();
    };
    ...
    this.inter_dadd = function(r, h, i) {
      ...
      rdres = rs1.add(rt1);
      r[this.rd(i)] = rdres.getLowBits();
      h[this.rd(i)] = rdres.getHighBits();
    };
    ...
    this.inter_ddiv = function(r, h, i) {
      ...
      mod = r1.modulo(r2);
      r[32] = res.getLowBits();
      h[32] = res.getHighBits();
      r[33] = mod.getLowBits();
      h[33] = mod.getHighBits();
    };
    this.inter_divu = function(r, h, i) {
      ...
      r[32] = (r[this.rs(i)] >>> 0) / (r[this.rt(i)] >>> 0);
      h[32] = 0;
      r[33] = (r[this.rs(i)] >>> 0) % (r[this.rt(i)] >>> 0);
      h[33] = 0;
    };
    this.inter_dmult = function(r, h, i) {
      ...
      if (num.length > 24) {
        delim = num.length - 24;
        h[33] = ("0x" + num.substr(0, delim)) >>> 0;
        r[33] = ("0x" + num.substr(delim, 8)) >>> 0;
        h[32] = ("0x" + num.substr(delim + 8, 8)) >>> 0;
        r[32] = ("0x" + num.substr(delim + 16, 8)) >>> 0;
      } else if (num.length > 16) {
        delim = num.length - 16;
        h[33] = 0;
        r[33] = ("0x" + num.substr(0, delim)) >>> 0;
        h[32] = ("0x" + num.substr(delim, 8)) >>> 0;
        r[32] = ("0x" + num.substr(delim + 8, 8)) >>> 0;
      } else if (num.length > 8) {
        delim = num.length - 8;
        h[33] = 0;
        r[33] = 0;
        h[32] = ("0x" + num.substr(0, delim)) >>> 0;
        r[32] = ("0x" + num.substr(delim, 8)) >>> 0;
      } else {
        delim = num.length;
        h[33] = 0;
        r[33] = 0;
        h[32] = 0;
        r[32] = ("0x" + num.substr(0, delim)) >>> 0;
      }
    };
    ...
    this.writeTLBEntry = function(tlb, cp0) {
      ...
      switch (tlb.pageMask) {
        case 0x00000000:
          tlb.loCompare = 0x00001000;
          break;
        case 0x00006000:
          tlb.loCompare = 0x00004000;
          break;
        case 0x0001e000:
          tlb.loCompare = 0x00010000;
          break;
        case 0x0007e000:
          tlb.loCompare = 0x00040000;
          break;
        case 0x001fe000:
          tlb.loCompare = 0x00100000;
          break;
        case 0x007fe000:
          tlb.loCompare = 0x00400000;
          break;
        case 0x01ffe000:
          tlb.loCompare = 0x01000000;
          break;
        default:
    ...
    this.buildTLBHelper = function(start, end, entry, mask, clear) {
      ...
      if (clear === true) {
        while (i < lend) {
          this.core.memory.physRegion[i] = (i & 0x1ffff) >>> 4;
          i++;
        }
      } else {
        ...
        while (i < lend) {
          ...
          this.core.memory.physRegion[i] = real >>> 16;
          i++;
        }
      }
    };
    this.buildTLB = function(tlb, clear) {
      ...
      lowest = (tlb.entryHi & 0xffffff00) >>> 0;
